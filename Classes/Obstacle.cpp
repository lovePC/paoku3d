...
void Obstacle::initDefault(Player* player,Node* render_node)
{
	...
	auto animation=Animation3D::create("model/piglet.c3b");
	auto animate=Animate3D::createWithFrames(animation,135,147);
	obstacle_sprite->runAction(RepeatForever::create(animate));
}

Sprite3D* Obstacle::getSpirte()
{
	return this->obstacle_sprite;
}

void Obstacle::bindTo(Node* render_node)
{
